...
{
private:
	static unique_ptr<SdlInstance> g_sdl;
...
public:
	static SdlInstance *get()
	{
		if (!g_sdl)
		{
			g_sdl.reset(new SdlInstance);
		}
		return g_sdl.get();
	}
...
unique_ptr<SdlInstance> SdlInstance::g_sdl;

extern JSMVariable<float> tick_time;

static int pollDevices(void *obj)
{
	while (SdlInstance::get()->keep_polling)
	{
		SDL_Delay(tick_time.get());

		std::lock_guard guard(SdlInstance::get()->controller_lock);
		for (auto iter = SdlInstance::get()->_controllerMap.begin(); iter != SdlInstance::get()->_controllerMap.end(); ++iter)
		{
			SDL_GameControllerUpdate();
			if (SdlInstance::get()->g_callback)
			{
				JOY_SHOCK_STATE dummy1;
				IMU_STATE dummy2;
				memset(&dummy1, 0, sizeof(dummy1));
				memset(&dummy2, 0, sizeof(dummy2));
				SdlInstance::get()->g_callback(iter->first, dummy1, dummy1, dummy2, dummy2, tick_time.get());
			}
			if (SdlInstance::get()->g_touch_callback)
			{
				TOUCH_STATE touch = JslGetTouchState(iter->first), dummy3;
				memset(&dummy3, 0, sizeof(dummy3));
				SdlInstance::get()->g_touch_callback(iter->first, touch, dummy3, tick_time.get());
			}
		}
	}
...
{
	if (!SdlInstance::get()->keep_polling)
	{
		SdlInstance::get()->keep_polling = true;

		SdlInstance::get()->_controller_polling_thread = SDL_CreateThread(pollDevices, "Poll Devices", nullptr);
		SDL_DetachThread(SdlInstance::get()->_controller_polling_thread);
	}
...
{
	std::lock_guard guard(SdlInstance::get()->controller_lock);
	auto iter = SdlInstance::get()->_controllerMap.begin();
	while (iter != SdlInstance::get()->_controllerMap.end())
	{
		delete iter->second;
		iter = SdlInstance::get()->_controllerMap.erase(iter);
	}
	for (int i = 0; i < size; i++)
	{
		...
		SdlInstance::get()->_controllerMap[handle] = device;
	}
	return SdlInstance::get()->_controllerMap.size();
}

void JslDisconnectAndDisposeAll()
{
	lock_guard guard(SdlInstance::get()->controller_lock);
	SdlInstance::get()->keep_polling = false;
	SdlInstance::get()->g_callback = nullptr;
	SdlInstance::get()->g_touch_callback = nullptr;
	auto iter = SdlInstance::get()->_controllerMap.begin();
	while (iter != SdlInstance::get()->_controllerMap.end())
	{
		delete iter->second;
...
IMU_STATE JslGetIMUState(int deviceId)
{
	...
	if (SdlInstance::get()->_controllerMap[deviceId]->has_gyro)
	{
		array<float, 3> gyro;
		SDL_GameControllerGetSensorData(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_SENSOR_GYRO, &gyro[0], 3);
	...
	if (SdlInstance::get()->_controllerMap[deviceId]->has_accel)
	{
		array<float, 3> accel;
		SDL_GameControllerGetSensorData(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_SENSOR_ACCEL, &accel[0], 3);
...
TOUCH_STATE JslGetTouchState(int deviceId, bool previous)
{
	...
	TOUCH_STATE state;
	if (SDL_GameControllerGetTouchpadFinger(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, 0, 0, &state0, &state.t0X, &state.t0Y, nullptr) == 0 &&
	  SDL_GameControllerGetTouchpadFinger(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, 0, 1, &state1, &state.t1X, &state.t1Y, nullptr) == 0)
	{
...
bool JslGetTouchpadDimension(int deviceId, int &sizeX, int &sizeY)
{
	...
	auto *jc = SdlInstance::get()->_controllerMap[deviceId];
	if (jc != nullptr)
	{
...
int JslGetButtons(int deviceId)
{
	...
	{
		buttons |= SDL_GameControllerGetButton(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_GameControllerButton(pair.first)) > 0 ? 1 << pair.second : 0;
	}
	switch (SdlInstance::get()->_controllerMap[deviceId]->split_type)
	{
	case SDL_CONTROLLER_TYPE_PS4:
	case SDL_CONTROLLER_TYPE_PS5:
		buttons |= SDL_GameControllerGetButton(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_BUTTON_TOUCHPAD) > 0 ? 1 << JSOFFSET_CAPTURE : 0;
		break;
	default:
		buttons |= SDL_GameControllerGetButton(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_BUTTON_MISC1) > 0 ? 1 << JSOFFSET_CAPTURE : 0;
		break;
	}
	return buttons;
}
...
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_LEFTX) / (float)SDL_JOYSTICK_AXIS_MAX;
}

float JslGetLeftY(int deviceId)
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_LEFTY) / (float)SDL_JOYSTICK_AXIS_MAX;
	;
}

float JslGetRightX(int deviceId)
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_RIGHTX) / (float)SDL_JOYSTICK_AXIS_MAX;
}

float JslGetRightY(int deviceId)
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_RIGHTY) / (float)SDL_JOYSTICK_AXIS_MAX;
}

float JslGetLeftTrigger(int deviceId)
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_TRIGGERLEFT) / (float)SDL_JOYSTICK_AXIS_MAX;
}

float JslGetRightTrigger(int deviceId)
{
	return SDL_GameControllerGetAxis(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_CONTROLLER_AXIS_TRIGGERRIGHT) / (float)SDL_JOYSTICK_AXIS_MAX;
}
...
float JslGetGyroX(int deviceId)
{
	...
	{
		float rawGyro[3];
		SDL_GameControllerGetSensorData(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, SDL_SENSOR_GYRO, rawGyro, 3);
	}
	return float();
}
...
bool JslGetTouchDown(int deviceId, bool secondTouch)
{
	...
	if (SDL_GameControllerGetTouchpadFinger(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, 0, secondTouch ? 1 : 0, &touchState, nullptr, nullptr, nullptr) == 0)
	{
		return touchState != 0;
	}
	return false;
...
{
	float x = 0;
	if (SDL_GameControllerGetTouchpadFinger(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, 0, secondTouch ? 1 : 0, nullptr, nullptr, &x, nullptr) == 0)
	{
		return x;
	}
	return x;
...
{
	float y = 0;
	if (SDL_GameControllerGetTouchpadFinger(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, 0, secondTouch ? 1 : 0, nullptr, nullptr, &y, nullptr) == 0)
	{
		return y;
	}
	return y;
...
{
	SdlInstance::get()->g_callback = callback;
}

void JslSetTouchCallback(void (*callback)(int, TOUCH_STATE, TOUCH_STATE, float))
{
	SdlInstance::get()->g_touch_callback = callback;
}

int JslGetControllerType(int deviceId)
{
	int type = SDL_GameControllerGetType(SdlInstance::get()->_controllerMap[deviceId]->_sdlController);
...
{
	return SdlInstance::get()->_controllerMap[deviceId]->split_type;
}
...
void JslSetLightColour(int deviceId, int colour)
{
	...
	uColour.raw = colour;
	SDL_GameControllerSetLED(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, uColour.argb[2], uColour.argb[1], uColour.argb[0]);
}

void JslSetRumble(int deviceId, int smallRumble, int bigRumble)
{
	SDL_GameControllerRumble(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, smallRumble << 8, bigRumble << 8, tick_time.get() + 1);
}

void JslSetPlayerNumber(int deviceId, int number)
{
	SDL_GameControllerSetPlayerIndex(SdlInstance::get()->_controllerMap[deviceId]->_sdlController, number);
}
